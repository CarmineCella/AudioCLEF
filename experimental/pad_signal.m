...
function y = pad_signal(x, Npad, boundary, center)
    ...
    has_imag = ~isreal(x);%sum(abs(imag(x(:))))>0;
	...
	for d = 1:length(Npad)
        ...
        % If has an imaginary part
        if has_imag 
        if ~strcmp(boundary,'zero')
                    gen_idx.type = '()';
        gen_idx.subs =repmat({':'},1,ndims(x));
        idx=gen_idx;
        idx.subs{d} = find(conjugate);
        cj = subsref (x,idx);
        x=subsasgn(x,idx,conj(cj));
        
       
       
%        x = subsasgn (x,idx,real(cj)-1i*imag(cj));
        %max(max(max(max(max(max(abs(Z-x)))))))
         %size(x)
         %size(Z)
%         
%             
%             %conjugate
%             x = x-2*1i*bsxfun(@times,conjugate.',imag(x));
        end
        end
        ...
        if strcmp(boundary,'zero')
            idx=gen_idx;
            idx.subs{d}=Norig+1:Npad(d);
            
            x = subsasgn (x,idx,0);%,cast(0,'like',x)); GPU
